...
export default function Box(props) {
    ...
    const [ updateControllerInput ] = useStore(state =>  [ state.updateControllerInput ], shallow)
    const id = mesh.current;
    const [ active, switchActive ]= useState(false)
    const [ initialized, switchInitialized ] = useState(false)
    const [ allEffects, getCI, effectExists ] = useStore(state => [state.allEffects, state.getCI, state.effectExists])
    ...
    if(id !== undefined && id.uuid !== undefined) {
      allEffects[id.uuid] = initialControllerInput;
    ...
    useFrame((frame) => {
      ...
      if(id !== undefined ) {cI = getCI(id.uuid)
